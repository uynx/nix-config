{
  ...
  # nixpkgs' `brave` is unbroken on aarch64-darwin, so it comes from there
  ...
  flake.homeModules.braveShortcuts = {
    ...
    # because the `brave` module is what makes the path exist, and this module
    # ships with it; a host without `web` gets neither.
