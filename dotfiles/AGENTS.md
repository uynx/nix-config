...
## Coding Guidelines
* **YAGNI & Conciseness (Source Code Only)**: When writing or implementing code inside source files or code blocks, strictly adhere to YAGNI ("You Aren't Gonna Need It") and implement code as concisely as possible (preferring clean one-liners where appropriate).
* **Prose Exception**: This conciseness constraint applies ONLY to generated code blocks. All prose explanation, reasoning, and textual answers must follow the Cognitive Performance Framework below, maximizing detail, depth, and length.
    </section>
